                    ...
                    if submit_fg:
                        ...
                        if st.session_state["edit_fg_id"] is not None:
                            cursor.execute(
                                ...
                                SET milling_id=?, production_date=?, miller_name=?, milling_qty=?, mfd_date=?, expiry_date=?, mrp=?, product_code=?, pouch_500g=?, pouch_1kg=?, pouch_2kg=?, pouch_5kg=?, total_finished_qty=?, bran_qty=?, bran_pct=?, refraction_qty=?, refraction_pct=?, yield_pct=?, processing_loss_pct=?
                                WHERE id=?
                            """,
                                (
                                    milling_id,
                            ...
                            cursor.execute(
                                """
                                INSERT INTO finished_goods (milling_id, production_date, miller_name, milling_qty, mfd_date, expiry_date, mrp, product_code, pouch_500g, pouch_1kg, pouch_2kg, pouch_5kg, total_finished_qty, bran_qty, bran_pct, refraction_qty, refraction_pct, yield_pct, processing_loss_pct)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                            """,
                                (
                                    milling_id,
...
# ... (Yahan se aage ka code, jaisa "4. Packing", "5. Dispatch" etc. aapke purane script mein hai)
