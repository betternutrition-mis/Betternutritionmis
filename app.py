...
if menu == "1. Dashboard":
    ...
    df_disp = load_data("daily_dispatch")
    
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("Milling Records", len(df_mil))
    with col2:
        st.metric("Finished Goods", len(df_fg))
    with col3:
        st.metric("Packing Material", len(df_pm))
    with col4:
        st.metric("Dispatch Records", len(df_disp))
    ...
    st.subheader("Recent Finished Goods Records")
    if not df_fg.empty:
...
# ==========================================
    ...
    with st.form("milling_form"):
        ...
        if submitted:
            if miller_name and milling_qty > 0:
                conn = get_connection()
                cursor = conn.cursor()
                cursor.execute(
                ...
                st.error("Kripya sabhi fields sahi se bharein.")

    st.subheader("Existing Milling Batches")
    df_mil_show = load_data("milling")
    if not df_mil_show.empty:
        st.dataframe(df_mil_show, use_container_width=True)
    else:
...
# ==========================================
    ...
    with st.form("pm_form"):
        ...
        item_name = st.text_input("Packing Material Item Name")
        quantity = st.number_input("Quantity", min_value=0, step=1)
        remarks = st.text_area("Remarks")
        
        pm_submit = st.form_submit_button("Save Packing Material Entry")
        if pm_submit:
            if item_name and quantity > 0:
                conn = get_connection()
                cursor = conn.cursor()
                cursor.execute(
                    "INSERT INTO packing_material (entry_date, item_name, quantity, remarks) VALUES (?, ?, ?, ?)",
                    (pm_date.strftime("%d %b %Y"), item_name, quantity, remarks)
                )
                conn.commit()
                conn.close()
                ...
                st.error("Kripya item name aur quantity sahi se bharein.")
...
# ==========================================
    ...
    with st.form("dispatch_form"):
        disp_date = st.date_input("Dispatch Date", value=datetime.date.today())
        party_name = st.text_input("Party Name")
        item_name = st.text_input("Item Name / SKU")
        quantity = st.number_input("Dispatch Quantity (kg/units)", min_value=0.0, step=1.0)
        vehicle_no = st.text_input("Vehicle Number")
        
        disp_submit = st.form_submit_button("Save Dispatch Entry")
        if disp_submit:
            if party_name and item_name and quantity > 0:
                conn = get_connection()
                cursor = conn.cursor()
                cursor.execute(
                    "INSERT INTO daily_dispatch (dispatch_date, party_name, item_name, quantity, vehicle_no) VALUES (?, ?, ?, ?, ?)",
                    (disp_date.strftime("%d %b %Y"), party_name, item_name, quantity, vehicle_no)
                )
                conn.commit()
                conn.close()
                st.success("Dispatch Entry Saved Successfully!")
                st.rerun()
            else:
                st.error("Kripya zaroori fields bharein.")

    st.subheader("Existing Dispatch Records")
    df_disp_show = load_data("daily_dispatch")
...
# ==========================================
# 6. MASTER RECORDS & EXPORT
# ==========================================
elif menu == "6. Master Records & Export (Admin)":
    st.header("Master Records & Data Export")
    
    tab1, tab2, tab3, tab4 = st.tabs(["Milling Master", "Finished Goods Master", "Packing Material Master", "Dispatch Master"])
    
    with tab1:
        st.subheader("Milling Master Table")
        df_m = load_data("milling")
        st.dataframe(df_m, use_container_width=True)
        
    with tab2:
        st.subheader("Finished Goods Master Table")
        df_f = load_data("finished_goods")
        st.dataframe(df_f, use_container_width=True)
        
    with tab3:
        st.subheader("Packing Material Master Table")
        df_p = load_data("packing_material")
        st.dataframe(df_p, use_container_width=True)
        
    with tab4:
        st.subheader("Daily Dispatch Master Table")
        df_d = load_data("daily_dispatch")
