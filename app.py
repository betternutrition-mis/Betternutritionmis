...
def get_miller_input(key_prefix, default_val=None):
    ...
    millers_list = df_emp["employee_name"].tolist() if not df_emp.empty and "employee_name" in df_emp.columns else ["Admin", "Miller Team"]
    ...
    with c2:
        with st.form("login_form"):
            ...
            if submitted:
                ...
                res = None
                if not df_emp.empty:
                    matched = df_emp[(df_emp["employee_name"] == username.strip()) & (df_emp["pin"] == pin.strip())]
...
if menu == "1. Raw Material Receiving":
    ...
    with st.form("raw_material_form"):
        ...
        with rc1:
            ...
            material_name = st.text_input("Material Name", placeholder="e.g. Wheat")
...
# ==========================================
    ...
    if df_rm_all.empty or "invoice_number" not in df_rm_all.columns:
        ...
        with st.form("raw_material_quality_form"):
            invoice_number = st.selectbox("Select Incoming Raw Material Invoice Number", invoices)
            
            qc1, qc2 = st.columns(2)
            with qc1:
                hl = st.number_input("HL (Hectolitre Weight)", min_value=0.0, step=0.1)
                foreign_material = st.number_input("Foreign Material %", min_value=0.0, step=0.01, format="%.2f")

            with qc2:
                moisture = st.number_input("Moisture %", min_value=0.0, step=0.1, format="%.1f")
...
# ==========================================
    ...
    with st.form("milling_form"):
        ...
        with mc2:
            material_type = st.text_input("Material Type", placeholder="e.g. Wheat Atta Grind")
            batch_code = st.text_input("Batch Code of Milling", placeholder="e.g. MILL-BATCH-01")
...
# ==========================================
    ...
    with st.form("finished_goods_form"):
        ...
        with fc1:
            ...
            use_obj = st.date_input("Use BY Date", value=datetime.date.today() + datetime.timedelta(days=90))
